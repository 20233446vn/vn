...
import { Building2, Users, Edit, Trash2, Plus, Briefcase, Search, X, Save } from 'lucide-react';
...
export default function DepartmentList() {
  const [departments, setDepartments] = useState<DepartmentState[]>(DEPARTMENTS);
  ...
  const enrichedDepartments = useMemo(() => {
    return departments.map(dept => {
      const employeeCount = EMPLOYEES.filter(e => e.MaPB === dept.MaPB).length;
      const manager = EMPLOYEES.find(e => e.MaPB === dept.MaPB && e.MaCV === 'CV02');
      
      return {
        ...dept,
        employeeCount,
        managerName: manager ? `${manager.HONV} ${manager.TENNV}` : 'Đang khuyết'
      };
    }).filter(d => d.TenPB.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [departments, searchTerm]);

  // Handlers
  const handleOpenAdd = () => {
    setCurrentDept(null);
    setFormData({ MaPB: `PB${departments.length + 1}`.padStart(4, '0'), TenPB: '' });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (currentDept) {
      // Edit
      setDepartments(prev => prev.map(d => d.MaPB === currentDept.MaPB ? formData : d));
    } else {
      // Add
  ...
  const handleDelete = () => {
    if (currentDept) {
      setDepartments(prev => prev.filter(d => d.MaPB !== currentDept.MaPB));
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Danh sách các đơn vị và phòng ban trong công ty</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {enrichedDepartments.map((dept) => (
          <div key={dept.MaPB} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-all duration-200 overflow-hidden group">
            <div className="p-6">
              ...
              <div className="mt-4">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{dept.TenPB}</h3>
                <p className="text-sm text-gray-500 font-mono mt-1">Mã: {dept.MaPB}</p>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-gray-50 dark:border-slate-700 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold text-gray-900 dark:text-gray-200">{dept.employeeCount} người</p>
                </div>
                <div>
                   <p className="text-xs text-gray-500 dark:text-gray-400 mb-1 flex items-center">
                  ...
                  <p className="font-semibold text-gray-900 dark:text-gray-200 truncate" title={dept.managerName}>{dept.managerName}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 dark:bg-slate-900/50 px-6 py-3 border-t border-gray-100 dark:border-slate-700 flex justify-between items-center">
              <span className="text-xs text-gray-500">Cập nhật: 01/01/2024</span>
              <button className="text-xs font-medium text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">Chi tiết &rarr;</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-sm w-full p-6 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">
              Bạn có chắc chắn muốn xóa phòng ban <span className="font-bold text-gray-800 dark:text-gray-200">"{currentDept?.TenPB}"</span>? Hành động này không thể hoàn tác.
            </p>
