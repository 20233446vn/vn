    ...
    async function load() {
      try {
        ...
        for (let d = 1; d <= daysCount; d++) {
          ...
          const checkInTime = record ? toHHmm(record.checkInTime) : null;
          const checkOutTime = record ? toHHmm(record.checkOutTime) : null;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        month,
        days: days
          // chỉ gửi những ngày có thông tin
          .filter(
            (d) =>
              d.status !== null || d.checkInTime || d.checkOutTime
          )
          .map((d) => ({
            date: d.dateStr,
            status: d.status,
                  ...
                  <tr key={d.day}>
                    ...
                    <td className="px-3 py-2">
                      <input
                        ...
                        value={d.checkInTime || ""}
                        onChange={(e) =>
                          handleTimeChange(d.day, "checkInTime", e.target.value)
                        }
                        className="px-2 py-1 rounded-md border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-xs"
                      />
                    </td>
