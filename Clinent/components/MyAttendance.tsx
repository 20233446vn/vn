...
interface MyAttendanceRecord {
  ...
  // Giờ vào / giờ ra từ DB
  check_in_time?: string | null;
  check_out_time?: string | null;

  // Giờ vào / ra đã format HH:mm từ API (nếu backend trả)
  checkInTime?: string | null;
  checkOutTime?: string | null;

  // Trạng thái riêng cho vào / tan (nếu backend có)
...
export default function MyAttendance() {
  ...
  const formatTime = (raw?: string | null): string => {
    if (!raw) return "—";
    // Nếu backend đã trả HH:mm thì dùng luôn
    ...
    // fallback nếu backend trả dạng '2025-12-04' không có time
  ...
  useEffect(() => {
    const now = new Date();
    setToday(formatDateDisplay(now.toISOString()));
  }, []);
  ...
  /* ---------------------------
     ...
     -> backend tự quyết định là VÀO hay TAN + status
  ---------------------------- */
  const markToday = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/attendance/mark-today`, {
        ...
        // Nếu muốn phân biệt nguồn chấm công, gửi thêm via: "FACE"
        body: JSON.stringify({ manv, via: "FACE" }),
      });
      ...
      setSuccessMsg(
        data.message ||
          `Đã chấm công hôm nay (${data.status || "thành công"}). Mã NV: ${
            data.manv || manv
          }.`
      );
      if (data.status) {
  ...
  const handleMarkByFace = async () => {
    ...
    try {
      ...
      // Ở đây không truyền status nữa, để backend tự tính giờ & trạng thái
      await markToday();
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="rounded-xl border bg-white p-4 flex items-center justify-between">
        <div>
          ...
          {/* Giờ vào / Giờ ra (thêm, nhưng vẫn trong card cũ -> không đổi layout) */}
          {(currentRecord?.checkInTime ||
            ...
            <div className="mt-2 text-xs text-gray-500 space-y-1">
              ...
              <p>
                ...
                {currentRecord?.check_out_status && (
                  <span className="ml-1">
                    ({currentRecord.check_out_status})
                  </span>
                )}
