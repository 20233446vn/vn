...
const InfoRow = ({
  ...
  <div className="py-3 sm:grid sm:grid-cols-3 sm:gap-4 border-b border-gray-100 dark:border-slate-700/70 last:border-0">
    <dt className="text-sm font-medium text-gray-500 dark:text-slate-400">
      {label}
    </dt>
    <dd className="mt-1 text-sm text-gray-900 dark:text-slate-100 sm:mt-0 sm:col-span-2 font-medium">
      {value && value !== "" ? value : "-"}
    </dd>
  </div>
);
...
export default function EmployeeDetail() {
  ...
  const [editingEvent, setEditingEvent] = useState<EmploymentEvent | null>(
    null
  );
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [empRes, deptRes, roleRes] = await Promise.all([
          ...
          fetch(`${API_BASE}/api/system-roles`),
        ]);
  ...
  const posName =
    roles.find((r) => r.MaPQ === employee.MaCV)?.TenPQ ||
  ...
  return (
    <div className="max-w-5xl mx-auto pb-10">
      <Link
        to="/employees"
        className="inline-flex items-center text-gray-500 dark:text-slate-400 hover:text-blue-600 mb-6 transition-colors"
      >
      ...
      {/* Header Card */}
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700/70 overflow-hidden mb-6">
        ...
        <div className="px-6 pb-6">
          <div className="relative flex items-end -mt-12 mb-6">
            <img
              src={avatarUrl}
              alt="Profile"
              className="w-24 h-24 rounded-xl border-4 border-white dark:border-slate-900 shadow-md object-cover bg-white dark:bg-slate-800"
            />
            <div className="ml-6 mb-1 flex-1">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-50">
                {employee.HONV} {employee.TENNV}
              </h1>
              <div className="flex items-center text-gray-600 dark:text-slate-300 mt-1">
              ...
              <span
                className={`px-3 py-1 text-sm font-semibold rounded-full ${
                  employee.Status === "Đang làm việc"
                    ...
                    : "bg-gray-100 text-gray-800 dark:bg-slate-600 dark:text-slate-50"
                }`}
              >
          ...
          {/* Quick Contact Info */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-100 dark:border-slate-700/70 pt-6">
            <div className="flex items-center text-sm text-gray-600 dark:text-slate-300">
              <Phone size={18} className="mr-3 text-gray-400 dark:text-slate-400" />
              {employee.DienThoai || "-"}
            </div>
            <div className="flex items-center text-sm text-gray-600 dark:text-slate-300">
              <Mail size={18} className="mr-3 text-gray-400 dark:text-slate-400" />
              {employee.Email || "-"}
            </div>
            <div className="flex items-center text-sm text-gray-600 dark:text-slate-300">
              <MapPin size={18} className="mr-3 text-gray-400 dark:text-slate-400" />
              {employee.DiaChi || "-"}
            </div>
          </div>
        </div>
      ...
      {isDueForReview && (
        <div className="bg-yellow-50 dark:bg-yellow-500/10 border-l-4 border-yellow-400 dark:border-yellow-500 p-4 mb-6 rounded-r-md flex items-start">
          <AlertCircle
            className="text-yellow-600 dark:text-yellow-300 mr-3 mt-0.5"
            size={20}
          />
          <div>
      ...
      {/* Tabs */}
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700/70 overflow-hidden min-h-[500px]">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex-1 py-4 px-1 text-center border-b-2 font-medium text-sm flex items-center justify-center gap-2 ${
                  activeTab === tab.id
                    ? "border-blue-500 text-blue-600 bg-blue-50/50 dark:bg-blue-500/10 dark:text-blue-300"
                    : "border-transparent text-gray-500 dark:text-slate-400 hover:text-gray-700 hover:border-gray-300 dark:hover:text-slate-100"
                }`}
              >
                {tab.icon}
                {tab.label}
        ...
        <div className="p-6">
          ...
          {activeTab === "profile" && (
            <div>
              <h3 className="text-lg font-medium leading-6 text-gray-900 dark:text-slate-50 mb-4">
                Thông tin cá nhân
              </h3>
              <dl>
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <button
                  ...
                  className="text-sm text-blue-600 dark:text-blue-300 font-medium hover:underline"
                >
                  + Thêm diễn biến
                </button>
              </div>
              ...
              {showEventForm && (
                <form
                  ...
                  className="mb-6 border border-gray-200 dark:border-slate-700/70 rounded-lg p-4 bg-gray-50 dark:bg-slate-800/60"
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100"
                        required
                      />
                    </div>
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 text-sm rounded-md border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-100 bg-white dark:bg-slate-900 hover:bg-gray-100 dark:hover:bg-slate-800"
                    >
                      Hủy
                    </button>
                    <button
              ...
              <div className="relative pl-6 border-l-2 border-gray-200 dark:border-slate-700 space-y-6">
                ...
                {events.map((ev) => (
                  <div key={ev.id} className="relative">
                    <div className="absolute -left-[31px] bg-blue-500 h-4 w-4 rounded-full border-2 border-white dark:border-slate-900"></div>
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <h4 className="text-sm font-bold text-gray-900 dark:text-slate-50">
                          {ev.title}
                        </h4>
                        <time className="block text-xs text-gray-500 dark:text-slate-400 mb-1">
                          {formatDate(ev.date)}
                        </time>
                        <p className="text-sm text-gray-600 dark:text-slate-200 whitespace-pre-line">
                          {ev.description}
                        </p>
                      </div>
          ...
          {activeTab === "education" && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 dark:text-slate-50 mb-4">
                Trình độ học vấn
              </h3>
              <dl className="mb-8">
                <InfoRow
              ...
              <h3 className="text-lg font-medium text-gray-900 dark:text-slate-50 mb-4 flex justify-between items-center">
              ...
              {/* Danh sách chứng chỉ */}
              <div className="border border-gray-200 dark:border-slate-700/70 rounded-md divide-y divide-gray-200 dark:divide-slate-700">
                {certLoading && (
                  <p className="text-sm text-gray-500 dark:text-slate-400 px-4 py-3">
                  ...
                  <div key={cert.id}>
                    <div className="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                      <div className="w-0 flex-1 flex items-center">
                        <BookOpen className="flex-shrink-0 h-5 w-5 text-gray-400 dark:text-slate-400" />
                        <div className="ml-2 flex-1 w-0">
                          <div className="flex items-center justify-between">
                            <span className="font-medium text-gray-900 dark:text-slate-50 truncate">
                              {cert.name}
                            </span>
                          </div>
                      ...
                      <div className="ml-4 flex-shrink-0 flex gap-3">
                        <button
                          type="button"
                          onClick={() => handleToggleViewCertificate(cert.id)}
                          className="text-xs text-blue-600 dark:text-blue-300 hover:underline"
                        >
                    ...
                    {expandedCertId === cert.id && (
                      <div className="px-6 pb-4 text-xs text-gray-700 dark:text-slate-200 bg-gray-50 dark:bg-slate-800/60">
                        {cert.note ? (
                          <p className="whitespace-pre-line">{cert.note}</p>
                        ) : (
                          <p className="italic text-gray-400 dark:text-slate-400">
          ...
          {activeTab === "salary" && (
            <div>
              ...
              <div className="mt-6">
                <h4 className="text-sm font-medium text-gray-900 dark:text-slate-50 mb-3">
                ...
                {salaryHistory.length > 0 && (
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead className="bg-gray-50 dark:bg-slate-900">
                      <tr>
                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-slate-300 uppercase">
                          Ngày hiệu lực
                        </th>
                        <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-slate-300 uppercase">
                          Lương cũ
                        </th>
                        <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-slate-300 uppercase">
                          Phụ cấp cũ
                    ...
                    <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-800">
                      {salaryHistory.map((row) => (
                        <tr key={row.id}>
                          ...
                          <td className="px-3 py-2 text-sm text-gray-900 dark:text-slate-100 text-right">
                          ...
                          <td className="px-3 py-2 text-sm text-gray-700 dark:text-slate-200 max-w-xs">
                            {row.note || "-"}
                          </td>
                        </tr>
                      ))}
