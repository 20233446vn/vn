...
interface Employee {
  ...
  DanToc?: string | null;
  TonGiao?: string | null;
  CMND?: string | null;             // dùng đúng tên cột trong DB
  HoKhau?: string | null;
...
export default function EmployeeDetail() {
  // ✅ dùng đúng tên param trong route: /employees/:manv
  const { manv } = useParams<{ manv: string }>();
  const [activeTab, setActiveTab] = useState<
    "profile" | "contracts" | "education" | "salary"
  >("profile");
        ...
        <div className="p-6">
          ...
          {activeTab === "salary" && (
            <div>
              ...
              {/* vẫn giữ giao diện mock, chưa nối API lương chi tiết */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-500">Lương cơ bản</p>
                  <p className="text-xl font-bold text-gray-900">
                    {Number(15000000).toLocaleString("vi-VN")} VNĐ
                  </p>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-500">Phụ cấp chức vụ</p>
                  <p className="text-xl font-bold text-gray-900">
                    {Number(3000000).toLocaleString("vi-VN")} VNĐ
                  </p>
                </div>
              </div>
